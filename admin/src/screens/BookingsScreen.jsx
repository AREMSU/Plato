...
const BookingsScreen = ({ navigation }) => {
  ...
  const renderBooking = ({ item }) => (
    <View style={styles.row}>
      ...
      <Text style={styles.sub}>{item.portions} portions · {formatCurrency(item.total_cost)}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.toolbar}>
        <TextInput style={styles.search} value={query} onChangeText={setQuery} placeholder="🔍 Search bookings..." placeholderTextColor={COLORS.textMuted} onSubmitEditing={load} />
        <View style={styles.pills}>
          {['', 'confirmed', 'cancelled'].map((s) => (
            <TouchableOpacity key={s} style={[styles.pill, statusFilter === s && styles.pillActive]} onPress={() => setStatusFilter(s)}>
              <Text style={[styles.pillText, statusFilter === s && styles.pillTextActive]}>{s || 'All'}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
      <FlatList data={bookings} keyExtractor={(item) => String(item.id)} renderItem={renderBooking} contentContainerStyle={{ padding: 16 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={COLORS.accent} />} ListEmptyComponent={<Text style={styles.empty}>No bookings found</Text>} />
    </View>
  );
};
